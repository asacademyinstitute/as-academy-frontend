...
function AdminCourseContentContent() {
    ...
    const handleDownload = async (lecture) => {
        if (!lecture.file_url) {
            showToast('No file URL available for this lecture', 'warning');
            return;
        }
        ...
        try {
            ...
            showToast('Failed to download lecture', 'error');
        }
    };
    ...
    const handleTransfer = async () => {
        if (selectedLectures.length === 0) {
            showToast('Please select at least one lecture to transfer', 'warning');
            return;
        }

        if (!targetChapterId) {
            showToast('Please select a target chapter', 'warning');
            return;
        }

        const confirmed = await customConfirm(`Are you sure you want to transfer ${selectedLectures.length} lecture(s) to the selected chapter?`, 'Confirm Transfer');
        if (!confirmed) {
            return;
        }
        ...
        try {
            ...
            showToast('Lectures transferred successfully!', 'success');
            setSelectedLectures([]);
            setTargetCourseId('');
            setTargetChapterId('');
            setTargetCourseChapters([]);
            ...
            showToast('Failed to transfer lectures', 'error');
        } finally {
                        ...
                        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                            ...
                            <div className="flex flex-col md:flex-row gap-3 w-full lg:w-auto items-stretch md:items-center">
                                ...
                                <select
                                    ...
                                    className="w-full md:w-64 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-ellipsis overflow-hidden"
                                >
                                ...
                                <button
                                    ...
                                    className="w-full md:w-auto px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors text-center font-medium"
                                >
